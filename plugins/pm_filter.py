...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, IS_VERIFY, SUPPORT_CHAT
...
async def pm_text(bot, message):
    ...
    await message.reply_text("<b>PLEASE SEARCH HERE👇🏼👇🏼👇🏼👇🏼\n\nhttps://t.me/+pX5IreUbBRY4OWM1\nhttps://t.me/+pX5IreUbBRY4OWM1\nhttps://t.me/+pX5IreUbBRY4OWM1\nhttps://t.me/+pX5IreUbBRY4OWM1</b>")
    await bot.send_message(
        chat_id=-1001932852728,
        text=f"<b>#𝐏𝐌_𝐌𝐒𝐆\n\nNᴀᴍᴇ : {user}\n\nID : {user_id}\n\nMᴇssᴀɢᴇ : {content}</b>"
    )
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                if NO_RESULTS_MSG:
                    await bot.send_message(chat_id=-1001892312478, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, movie)))
    ...
    if query.data.startswith("file"):
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                             [
                              ...
                              InlineKeyboardButton("Bᴏᴛ Oᴡɴᴇʀ", url="t.me/shortnfly_support")
                             ]
                            ]
                        )
                    )
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/IamDora4")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    if NO_RESULTS_MSG:
                        await client.send_message(chat_id=-1001892312478, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, search)))
                    return
        else:
            return
    else:
...
async def advantage_spell_chok(client, msg):
    ...
    try:
        ...
        if NO_RESULTS_MSG:
            await client.send_message(chat_id=-1001892312478, text=(script.NORSLTS.format(reqstr.id, reqstr.mention, mv_rqst)))
